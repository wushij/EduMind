-- -----------------------------------------------------------------------------
-- V2.5.1: 深度思考 BC 合体提示词（全局 1～4 条见 AiPromptConstants；本迁移为 chat_rag 追加 1～5 条）
-- -----------------------------------------------------------------------------
...
UPDATE prompt_template SET
    ...
    update_time = NOW()
WHERE code = 'chat_rag'
  AND (system_prompt IS NULL OR system_prompt NOT LIKE '%【深度思考与输出预算（高优先级）】%');
...
WHERE pt.code = 'chat_rag'
  AND pt.system_prompt LIKE '%【深度思考与输出预算（高优先级）】%';
