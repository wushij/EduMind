  ...
  <div class="ai-marketplace-page">
    ...
    <div class="tools-grid-section">
      ...
      <div v-else class="empty-box">
        <el-icon class="empty-icon-svg"><School /></el-icon>
        <h4>暂无教师类 AI 工具</h4>
        <p>可尝试切换分类或搜索其他工具</p>
      </div>
    </div>
...
onMounted(() => fetchTools('TEACHER'));
watch(searchKeyword, (kw) => fetchTools('TEACHER', kw.trim() || undefined));

const filteredTools = computed(() => {
  const list = tools.value.filter(t => t.category === 'TEACHER');
...
function handleSearch(val: string) {
  ...
  fetchTools('TEACHER', val.trim() || undefined);
}
