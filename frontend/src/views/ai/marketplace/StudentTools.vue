  ...
  <div class="ai-marketplace-page">
    ...
    <div class="tools-grid-section">
      ...
      <div v-else class="empty-box">
        <el-icon class="empty-icon-svg"><Reading /></el-icon>
        <h4>暂无学生类 AI 工具</h4>
        <p>可尝试切换分类或搜索其他工具</p>
      </div>
    </div>
...
onMounted(() => fetchTools('STUDENT'));
watch(searchKeyword, (kw) => fetchTools('STUDENT', kw.trim() || undefined));

const filteredTools = computed(() => {
  const list = tools.value.filter(t => t.category === 'STUDENT');
...
function handleSearch(val: string) {
  ...
  fetchTools('STUDENT', val.trim() || undefined);
}
