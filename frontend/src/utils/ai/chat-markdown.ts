...
function normalizeInlineNumberedLists(text: string): string {
  return text
    ...
    .map((segment) => {
      ...
      // 1. 若同一行前文粘连了带加粗的序号小标题（如「...说明机制。**1. 异常体系**」），换行符必须置于 ** 之前
      s = s.replace(
        /([^\n])[ \t]*\*\*(\d{1,2})\.(?=[ \t]*)/g,
        '$1\n\n**$2. '
      );
      ...
      // 3. 普通无加粗序号拆行：负向预查排除数字、换行符和星号 *（避免切断加粗标记，避免误伤 3.14 小数）
      s = s.replace(
        /(?<![\d\n*])(\d{1,2})\.(?=[ \t]*[\p{Extended_Pictographic}\u4e00-\u9fa5（(「『【A-Za-z])/gu,
        (_match, num: string, offset: number) => `${offset > 0 ? '\n' : ''}${num}. `
      );
 ...
 * 全面支持 Emoji 图标（如 - 📘 概念、- ✍️ 分析、- 🔍 答疑、- 🧮 练习）及句末交流引导句自动分段。
 */
function normalizeInlineBulletLists(text: string): string {
  return text
    ...
    .map((segment) => {
      ...
      // 例如：含义与区别- ✍️**推导分析**： 或 递推式- 🔍**答疑解惑**： 或 可以帮你：- 📘
      s = s.replace(
        /(?<!-)([\u4e00-\u9fa5。；;!?！？:：）)”"』」】])\s*-(?:\s*|\s+)(?=[\p{Extended_Pictographic}\*\*\u4e00-\u9fa5【「『（])/gu,
        '$1\n- '
      );

      // 3. 段末/行中同一行粘连 -列表（仅中文/Emoji/** 开头），必须排除减号与换行，且空格仅限行内空白（避免误伤 --- 分割线与 1 - cos x）
      s = s.replace(
      ...
      // 5. 列表项末尾粘连的交流互动引导句（如「…答案不妨先聊聊：」「…解答那么我们…」）自动脱离列表形成独立自然段
      s = s.replace(
...
/**
 * 教学与问答高频小标题（如「**参考答案：**」「参考解析：」「【题目】」等）粘连自动拆行
 */
function normalizeTeachingHeaders(text: string): string {
  ...
  return text
    ...
    .map((segment) => {
      if (segment.startsWith('```')) return segment;
      let s = segment.replace(HEADER_RE, '$1\n\n$2');
